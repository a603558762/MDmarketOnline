var vm = new Vue({
    ...
    mounted: function () {
        ...
        // 调用
        axios.get(this.hosts + "/addresses/", {
            headers: {
    ...
    watch: {
        'form_address.province_id': function () {
            if (this.form_address.province_id) {
                axios.get(this.hosts + '/areas/' + this.form_address.province_id + '/', {
                    responseType: 'json'
                })
                    .then(response => {
        ...
        'form_address.city_id': function () {
            if (this.form_address.city_id) {
                axios.get(this.hosts + '/areas/' + this.form_address.city_id + '/', {
                    responseType: 'json'
                })
                    .then(response => {
    ...
    methods: {
        ...
        save_address: function () {
            if (this.error_receiver || this.error_place || this.error_mobile || this.error_email || !this.form_address.province_id || !this.form_address.city_id || !this.form_address.district_id) {
                ...
                if (this.editing_address_index === '') {
                    // 新增地址
                    axios.post(this.hosts + '/addresses/', this.form_address, {
                        headers: {
                            'Authorization': 'JWT ' + this.token
                        },
                        responseType: 'json'
                    ...
                    // 修改地址
                    axios.put(this.hosts + '/addresses/' + this.addresses[this.editing_address_index].id + '/', this.form_address, {
                        headers: {
                            'Authorization': 'JWT ' + this.token
                        },
                        responseType: 'json'
        ...
        del_address: function (index) {
            axios.delete(this.hosts + '/addresses/' + this.addresses[index].id + '/', {
                headers: {
                    'Authorization': 'JWT ' + this.token
                },
                responseType: 'json'
        ...
        set_default: function (index) {
            axios.put(this.hosts + '/addresses/' + this.addresses[index].id + '/status/', {}, {
                headers: {
                    'Authorization': 'JWT ' + this.token
                },
                responseType: 'json'
        ...
        save_title: function (index) {
            if (!this.input_title) {
                ...
                axios.put(this.hosts + '/addresses/' + this.addresses[index].id + '/title/', {
